    ...
    (set, get) => ({
      ...
      checkAuth: async () => {
        ...
        try {
          ...
          const refreshToken = (await getRefreshToken()) || get().refreshToken;
          const response = await authService.getCurrentUser();
          if (response.success && response.data) {
            set({ user: response.data, token, userToken: token, refreshToken, isLoading: false });
          } else {
            set({ user: null, token: null, userToken: null, refreshToken: null, isLoading: false });
          }
        } catch (e) {
      ...
      logout: async () => {
        ...
        try {
          ...
          set({ user: null, token: null, userToken: null, refreshToken: null, error: null, isLoading: false });
        }
      },
